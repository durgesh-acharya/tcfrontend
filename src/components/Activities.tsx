...
interface ActivitiesProps {
  activityData: { day: number; title: string; isticketinclude: number }[];  // Inline type definition for the prop
}

const Activities : React.FC<ActivitiesProps> = ({ activityData}) => {
        ...
        {activityData.map((item, index) => (
          <div key={index} className="bg-white rounded-lg shadow-md">
            ...
            <div className="flex items-center justify-between p-4 cursor-pointer" onClick={() => toggleExpand(index)}>
              ...
              <div className="text-gray-900 text-xs font-bold">{item.title}</div>
            ...
            {expanded === index && (
              <div className="p-4 border-t border-gray-300 bg-white">
                <p className="text-gray-600">{item.isticketinclude}</p>
              </div>
            )}
          </div>
        ))}
