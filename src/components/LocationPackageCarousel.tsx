...
const LocationPackageCarousel: React.FC<{ location: Location }> = ({ location }) => {
  ...
  const handleScroll = (direction: 'left' | 'right') => {
    const maxIndex = location.packages.length - 3; // 3 packages visible at a time (this will change based on screen size)
  ...
  return (
    <div className="container mx-auto my-6">
      ...
      <div className="relative overflow-hidden"> {/* Add overflow-hidden here */}
        ...
        >
          {location.packages.slice(scrollIndex, scrollIndex + (windowWidth <= 640 ? 1 : 3)).map((pkg) => (
